...
export function BookTable() {
  ...
  const rows =
    view === "overdue"
      ? books.filter((b) => {
          const c = getCheckoutForBook(b.id);
  ...
  return (
    <>
            ...
            {rows.map((book) => {
              ...
              return (
                <tr key={book.id} className="transition-colors hover:bg-gray-50/80">
                  ...
                  <td className="hidden px-4 py-3 text-gray-500 md:table-cell">
                    {active
                      ? new Date(active.dueDate).toLocaleDateString("en-US", {
                          month: "short",
                          day: "numeric",
                        })
                      : "\u2014"}
          ...
          return (
            ...
            >
              ...
              <div className="mt-3 flex items-center justify-between border-t border-gray-100 pt-3">
                {available ? (
                  <Button className="w-full" onClick={() => setCheckingOut(book)}>
                    Check out
                  </Button>
                ) : (
                  <>
      ...
      {/* Checkout modal — lazy loaded, only fetched when needed */}
      {checkingOut && (
